...
class CCS200Linux:
    ...
    def __init__(self, serial_number: Optional[str] = None, device_index: int = 0):
        ...
        if not PYUSB_AVAILABLE:
            raise ImportError(
                "pyusb is required for Linux CCS200 support. "
    ...
    def _connect(self):
        ...
        devices = list(usb.core.find(
            find_all=True,
            idVendor=self.VID,
            idProduct=CCS200_PID
        ))
        
        # Also check alternate PID
        devices_alt = list(usb.core.find(
            find_all=True,
            idVendor=self.VID,
            idProduct=CCS200_PID_ALT
        ))
    ...
    def _send_usbtmc_command(self, command: bytes, expect_response: bool = True) -> Optional[bytes]:
        ...
        header_in = bytes([
            msg_id_in, self._btag, btag_inv, 0x00,
            transfer_size & 0xFF,
    ...
    def _load_wavelengths(self):
        ...
        try:
            ...
            print(f"  ✓ Wavelength range: {self._wavelengths[0]:.1f} - {self._wavelengths[-1]:.1f} nm")
...
if __name__ == "__main__":
    ...
    try:
        with CCS200Linux() as spec:
            ...
            print(f"Wavelengths: {spec.get_wavelengths()[0]:.1f} - {spec.get_wavelengths()[-1]:.1f} nm")
