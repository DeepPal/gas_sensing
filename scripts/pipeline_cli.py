...
def main():
    ...
    parser.add_argument(
        "--data", required=True, help="Experiment root directory (contains concentration folders)"
    )
    ...
    parser.add_argument(
        "--sweep", action="store_true", help="Run hyperparameter sweep instead of single pipeline"
    )
    parser.add_argument(
        "--avg-top-n",
        type=int,
    ...
    parser.add_argument(
        "--params", type=str, help="JSON string or path to JSON defining ROI parameter grid"
    )
    parser.add_argument(
        "--dataset-map",
        type=str,
    ...
    if args.sweep:
        ...
        if args.dataset_map:
            with open(args.dataset_map) as f:
                datasets = json.load(f)
            if not isinstance(datasets, list):
                parser.error(
                    '--dataset-map must be a JSON list of {"data", "ref", optional "label"}'
                )
        else:
            datasets = [
                {
    ...
    metadata.get("multi_signal", {}) if isinstance(metadata, dict) else {}
    minimal_outputs = (
        bool(
            ((metadata.get("roi_config", {}) or {}).get("shift", {}) or {}).get(
    ...
    if multi_sig:
        ...
        print("-------------------------")
        sig_map = (
            multi_sig.get("signals", {}) if isinstance(multi_sig.get("signals", {}), dict) else {}
        )
        available_signals = (
            multi_sig.get("available_signals", [])
            if isinstance(multi_sig.get("available_signals", []), list)
            else []
        )
        ...
        for sig, info in sig_map.items():
            ...
            print(f"  • {sig}")
            print(
                f"      window      : {best_row.get('start_nm', 'NA')}–{best_row.get('end_nm', 'NA')} nm"
            )
        ...
        cross = (
            multi_sig.get("cross_signal", {})
            if isinstance(multi_sig.get("cross_signal", {}), dict)
            else {}
        )
        if cross:
    ...
    metadata_path = (
        result.get("run_metadata") or metadata.get("run_metadata")
        if isinstance(metadata, dict)
        else None
    )
    if metadata_path:
    ...
    report_path = (
        result.get("report_artifacts", {}).get("summary_markdown")
        if isinstance(result.get("report_artifacts"), dict)
        else None
    )
    if report_path:
    ...
    if args.predict_dir:
        print("\nBatch prediction")
        print("----------------")
        pred_path = pipeline_mod.predict_batch_with_selected_model(
            args.predict_dir, ref_path, out_root
        )
        if pred_path:
