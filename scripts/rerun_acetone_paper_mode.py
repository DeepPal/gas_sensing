...
def main():
    ...
    data_dir = Path("Joy_Data/Acetone")
    ppm50_dir = data_dir / "50ppm"
    pd.read_csv("Joy_Data/Acetone/air1.csv", header=0, names=["wavelength", "intensity"])
    ...
    try:
        ...
        cmd = [
            sys.executable,
            "-m",
            "gas_analysis.core.run_each_gas",
            "--gas",
            "Acetone",
            "--avg-top-n",
            "6",
        ...
        print(
            f"  Difference: {abs(slope - paper_slope):.6f} nm/ppm ({abs(slope - paper_slope) / paper_slope * 100:.1f}%)"
        )
        print()
        ...
        print("Calibration (canonical peaks):")
        ...
        if abs(slope - paper_slope) < 0.03:
            ...
            print(
                f"⚠️ Slope differs from paper by {abs(slope - paper_slope) / paper_slope * 100:.1f}%"
            )
            print("\nPossible reasons:")
