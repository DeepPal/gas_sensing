...
def load_spectral_data(
    ...
    **kwargs,
    ...
    try:
        ...
        data = pd.read_csv(file_path, skiprows=skiprows, header=header, names=names, **kwargs)
...
def _format_experiment_dir(
    concentration: str, run: Union[int, str], template: Optional[str] = None
) -> str:
...
def load_reference_data(
    base_dir: Union[str, Path], gas_type: str = None
    ...
    if gas_type is not None:
        ...
        if gas_type not in ref_files:
            raise ValueError(
                f"Invalid gas_type: {gas_type}. Must be one of {list(ref_files.keys())}"
            )
