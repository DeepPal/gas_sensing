...
class RealtimeAcquisitionService:
    ...
    def __init__(
        ...
        # Statistics
        self.stats = {"dropped_samples": 0, "avg_acquisition_time": 0.0, "start_time": None}
    ...
    def connect(self, interface: str = "auto"):
        ...
        self._interface_used = (
            interface if interface != "auto" else (self._interface_used or "native")
        )
    ...
    def _try_interface(self, interface: str):
        ...
        if interface == "native":
            ...
            for attempt in range(attempts):
                try:
                    self.spectrometer = CCS200Spectrometer(
                        resource_string=self.resource_string,
                        integration_time_s=self.integration_time_ms / 1000.0,
                    )
                    break
                except RuntimeError as exc:
                    last_exc = exc
                    if attempt == 0 and self._should_retry_native(exc):
                ...
                if full_spectrum:
                    ...
                    for d in self.data_buffer:
                        row = [
                            d["sample_num"],
                            d["timestamp"],
                            self.wavelengths[self.target_idx],
                            d["target_intensity"],
                        ]
    ...
    def auto_select_wavelength(
        self, approx_wavelength: Optional[float] = None, window_nm: float = 5.0
    ):
...
def main():
    ...
    service = RealtimeAcquisitionService(
        integration_time_ms=INTEGRATION_MS, target_wavelength=TARGET_WAVELENGTH
    )
        ...
        def file_logger(sample):
            ...
            if log_to_csv and csv_writer:
                ...
                row = [
                    sample["sample_num"],
                    sample["timestamp"],
                    service.wavelengths[service.target_idx],
                    sample["target_intensity"],
                    float(intensities.mean()),
                    float(intensities.max()),
                    float(intensities.min()),
                ]
