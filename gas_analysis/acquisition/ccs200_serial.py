...
class CCS200Serial:
    ...
    def _load_wavelengths(self):
        ...
        try:
            ...
            response = self._send_command(self.CMD_GET_WAVELENGTHS, read_length=self.NUM_PIXELS * 8)
    ...
    def get_scan_data(self) -> np.ndarray:
        ...
        for _attempt in range(3):
            response = self._send_command(self.CMD_GET_DATA, read_length=expected_bytes)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\nDevice Info:")
        print(
            f"  Wavelength range: {spec.get_wavelengths()[0]:.1f} - {spec.get_wavelengths()[-1]:.1f} nm"
        )
        print(f"  Number of pixels: {len(spec.get_wavelengths())}")

        # Set integration time
        spec.set_integration_time(0.01)  # 10ms
        print("  Integration time: 10 ms")
