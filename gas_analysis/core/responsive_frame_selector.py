...
def compute_response_metric(
    ...
    if compute_response_metric.debug_count <= 1:
        ...
        print(
            f"  Intensity stats: min={intensity.min():.4f}, max={intensity.max():.4f}, mean={intensity.mean():.4f}"
        )
        print(
            f"  Ref stats: min={ref_intensity.min():.4f}, max={ref_intensity.max():.4f}, mean={ref_intensity.mean():.4f}"
        )
    ...
    if compute_response_metric.debug_count <= 1:
        ...
        if len(A_roi) > 0:
            print(
                f"  A_roi stats: min={A_roi.min():.4f}, max={A_roi.max():.4f}, mean={A_roi.mean():.4f}"
            )
...
def find_responsive_frames(
    ...
    if smooth_window > 1 and n_frames >= smooth_window:
        smoothed = np.convolve(
            response_metrics, np.ones(smooth_window) / smooth_window, mode="same"
        )
    else:
    ...
    metadata = {
        ...
        "response_enhancement": float(np.mean(selected_metrics) / np.mean(response_metrics))
        if np.mean(response_metrics) > 0
        else 1.0,
    ...
    print(
        f"  Range: frames {top_indices[0]} to {top_indices[-1]} (span: {metadata['selection_span_frames']} frames)"
    )
    print(
        f"  Mean response: {metadata['selected_mean_response']:.3f} (enhancement: {metadata['response_enhancement']:.2f}x)"
    )
...
def average_frames_intensity(
    frames: list[pd.DataFrame], frame_indices: list[int], method: str = "mean"
) -> pd.DataFrame:
    """
    ...
    for frame in selected_frames:
        intensity = np.interp(
            base_wavelength, frame["wavelength"].values, frame["intensity"].values
        )
    ...
    result = pd.DataFrame({"wavelength": base_wavelength, "intensity": avg_intensity})
...
if __name__ == "__main__":
    ...
    # Load air reference
    air_ref = pd.read_csv("Joy_Data/Acetone/air1.csv", header=0, names=["wavelength", "intensity"])

    # Load all frames from a trial
    trial_path = Path("Joy_Data/Acetone/3ppm/T1")
    ...
    visualize_time_series(
        time_series,
        title="3ppm Trial 1 Response",
        save_path=Path("test_time_series.png"),
    )
    ...
    print(
        f"Wavelength range: {avg_spectrum['wavelength'].min():.2f} - {avg_spectrum['wavelength'].max():.2f} nm"
    )
