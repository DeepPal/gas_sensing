...
def compute_t90_t10_from_timeseries(
    ...
    if not csv_files:
        ...
        return {'summary': {}, 'results': pd.DataFrame(), 'json_path': None, 'csv_path': None}
    ...
    for csv_file in sorted(csv_files):
        ...
        if len(delta) < baseline_frames + steady_state_frames:
            print(f"[WARNING] Not enough frames in {csv_file} (need {baseline_frames + steady_state_frames})")
            continue
        ...
        if frame_rate is None:
            ...
            current_frame_rate = max(0.1, min(10.0, estimated_frame_rate))  # Clamp to reasonable range
        ...
        try:
            amp_thresh = float(min_response_amplitude_nm) if min_response_amplitude_nm is not None else 0.0
        except Exception:
            amp_thresh = 0.0
        amp_thresh = max(amp_thresh, 1e-6)

        if response_amplitude < amp_thresh:
            print(f"[WARNING] Response amplitude {response_amplitude:.4g} nm below threshold {amp_thresh:.4g} nm in {csv_file}")
            t90 = float('nan')
            t10 = float('nan')
        else:
        ...
        try:
            ...
            ax.axhline(steady_state, color='tab:green', linestyle='--', linewidth=0.9, label='steady-state')
    ...
    print(f"[INFO] T90 (response): {summary.get('T90_mean_s', 'N/A'):.2f} ± {summary.get('T90_std_s', 0):.2f} s")
    print(f"[INFO] T10 (recovery): {summary.get('T10_mean_s', 'N/A'):.2f} ± {summary.get('T10_std_s', 0):.2f} s")
