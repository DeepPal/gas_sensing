...
def detect_active_segments(
    ...
    """Detect segments where signal exceeds threshold (with hysteresis).

    Args:
        values: 1D signal array
        high_threshold: Threshold to start segment
        low_threshold: Threshold to end segment
        min_length: Minimum segment length
        pad: Padding to add to segments
...
def pelt_changepoint_detection(
    signal: np.ndarray, penalty: float = 3.0, min_size: int = 5
) -> list[int]:
    """
