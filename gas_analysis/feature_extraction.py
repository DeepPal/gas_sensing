...
try:
    from scipy.integrate import simpson as simps  # scipy >= 1.14
except ImportError:
    from scipy.integrate import simps  # type: ignore[no-redef]  # scipy < 1.14
...
def extract_peak_features(
    spectrum: pd.DataFrame, peaks_info: dict[str, np.ndarray]
) -> pd.DataFrame:
    """
    ...
    for i, peak in enumerate(peaks):
        ...
        fwhm = properties["widths"][i] * (
            spectrum["wavelength"].iloc[1] - spectrum["wavelength"].iloc[0]
        )
...
def extract_all_features(spectrum: pd.DataFrame, peaks_info: dict[str, np.ndarray]) -> dict:
    """
    ...
    features = {"num_peaks": len(peaks_info["peaks"]), **stats_features}
