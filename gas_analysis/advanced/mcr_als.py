...
def save_mcr_components_overlay(
    ...
    try:
        ...
        if comps.size and wl.size and comps.shape[1] == wl.size:
            ...
            plt.xlabel("Wavelength (nm)")
            plt.ylabel("MCR-ALS Component (arb.)")
            plt.title("MCR-ALS Component Spectra (ROI overlay)")
            plt.grid(True, alpha=0.3)
            plt.legend(ncol=2, fontsize=8)
            plt.tight_layout()
            out_path = str(plots_dir / "mcr_als_components_roi.png")
