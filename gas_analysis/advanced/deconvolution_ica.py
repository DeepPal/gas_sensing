...
def _build_feature_matrix_from_canonical(
...
def fit_ica_from_canonical(
    canonical: dict[float, pd.DataFrame], cfg: dict[str, object]
...
def save_ica_outputs(
    res: dict[str, object], out_root: str, roi: Optional[tuple[float, float]] = None
) -> dict[str, str]:
    metrics_path = None
    comp_plot_path = None
    ...
    try:
        ...
        try:
            ...
            if S.size and y.size and S.shape[0] == y.size and bi < S.shape[1]:
                ...
                mn, mx = (
                    float(min(y.min(), yhat.min())),
                    float(max(y.max(), yhat.max())),
                )
...
def save_ica_components_overlay(
    res: dict[str, object], out_root: str, roi: tuple[float, float]
) -> Optional[str]:
    try:
        from pathlib import Path
        ...
        if comps.size and wl.size and comps.shape[1] == wl.size:
            ...
            plt.legend(ncol=2, fontsize=8)
            plt.tight_layout()
            out_path = str(plots_dir / "ica_components_roi.png")
