...
class GasAnalyzer:
    ...
    def find_peaks(self, wavelengths: np.ndarray, spectrum: np.ndarray, **kwargs) -> list[Peak]:
        """
        ...
        peak_indices, properties = signal.find_peaks(
            spectrum, prominence=prominence, width=width, distance=distance
        )
        ...
        for i, idx in enumerate(peak_indices):
            self.peaks.append(
                Peak(
                    ...
                    left_base=wavelengths[properties["left_bases"][i]]
                    if "left_bases" in properties
                    else 0,
                    right_base=wavelengths[properties["right_bases"][i]]
    ...
    def analyze(
        self,
        data: Union[pd.DataFrame, tuple[np.ndarray, np.ndarray], dict[str, np.ndarray]],
    ) -> dict[str, pd.DataFrame]:
        if isinstance(data, pd.DataFrame):
            raw = data.copy()
        elif isinstance(data, (tuple, list)) and len(data) >= 2:
            raw = pd.DataFrame(
                {"wavelength": np.asarray(data[0]), "intensity": np.asarray(data[1])}
            )
