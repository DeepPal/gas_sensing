        ...
        if bs and bs.get("enabled") and isinstance(bs.get("ci", {}), dict):
            ...
            try:
                ...
                if np.isfinite(lod_val) and np.isfinite(loq_val):
                    ...
                    md.append(
                        f"Using the ROI slope and noise estimates, LOD ≈ {lod_val:.3g} ppm and LOQ ≈ {loq_val:.3g} ppm."
                    )
            except Exception:
                pass
    else:
        ...
        try:
            ...
            try:
                ...
                if "slope_ci_low" in head.columns and "slope_ci_high" in head.columns:
                    notes.append(
                        "slope_ci_low/high are 95% t-based confidence interval bounds for the slope."
                    )
                notes.append(
                    "Slope units are ΔT/ppm; higher |slope| and STN indicate stronger, more detectable effects."
                )
                if notes:
        ...
        if cr and isinstance(cr, dict):
            ...
            if mt and mt.get("enabled"):
                ...
                md.append(
                    _md_kv_table(
                        mt,
                        ["method", "alpha", "significant_count"],
                        ["Method", "Alpha", "Significant wavelengths"],
                    )
                )
                md.append("")
            ...
            if pt and pt.get("enabled"):
                ...
                md.append(
                    _md_kv_table(
                        ...
                        ["Iterations", "Observed |slope|", "Empirical p-value"],
                    )
                )
                pct = pt.get("percentiles", {})
                if pct:
                    md.append(
                        f"Percentiles (5/50/95): {pct.get('5', 'N/A')}/{pct.get('50', 'N/A')}/{pct.get('95', 'N/A')}"
                    )
                md.append("")
                md.append(
                    "Note: In the Volcano and Manhattan plots, the dashed horizontal line marks the FDR threshold (q = α)."
                )
                md.append(
                    "Note: In the Volcano and Manhattan plots, the dashed horizontal line marks the FDR threshold (q = α)."
                )
...
def _interpretation_md(run_dir: Path) -> str:
    ...
    md.append(
        "This guide explains what each figure/table conveys, why it's used, and how to read it.\n\n"
    )
    items = [
        (
            "Canonical overlay",
            "Overlay of aggregated spectra per concentration. Why: sanity-check directionality and shape of response within ROI. Read: look for monotonic changes and consistent band shapes across concentrations.",
        ),
        (
            "Concentration response",
            "ROI signal vs concentration with regression. Why: quantify sensitivity and linearity. Read: slope sign/magnitude, R², RMSE; compare to LOD/LOQ.",
        ),
        (
            "Full-scan concentration response",
            "Effect profiles across all wavelengths. Why: locate bands with strongest trends. Read: peaks indicate high effect bands; used to seed ROI.",
        ),
        (
            "Slope-to-noise heatmap",
            "Effect size normalized by noise. Why: emphasizes robust detectability. Read: warmer colors mean higher slope-to-noise; thresholds suggest reliable bands.",
        ),
        (
            "Volcano plot",
            "Effect (slope) vs −log10(q). Why: combine magnitude and significance under FDR control. Read: upper points are more significant; right/left reflect effect direction.",
        ),
        (
            "Manhattan plot",
            "−log10(q) across wavelength. Why: identify continuous significant regions. Read: spikes highlight statistically robust bands after multiple-testing control.",
        ),
        (
            "Permutation summary",
            "Observed |slope| vs empirical null percentiles. Why: nonparametric validation of ROI effect. Read: observed line beyond 95th percentile → unlikely by chance.",
        ),
        (
            "Top wavelengths table",
            "Band-level stats (slope, r², p/q, significance). Why: tabular view of best bands. Read: use adjusted p (q) and slope CI to prioritize interpretable, stable peaks.",
        ),
        (
            "PLSR CV curves",
            "R²/RMSE across components. Why: select complexity via one-SE rule. Read: prefer parsimonious component count with near-best CV performance.",
        ),
        (
            "PLSR predicted vs actual",
            "Calibration scatter with ideal line. Why: assess bias and spread. Read: points hugging y=x and narrow spread indicate good calibration.",
        ),
        (
            "PLSR residuals / histogram",
            "Error structure and magnitude. Why: check assumptions and define empirical predictive intervals. Read: symmetric, centered residuals imply well-behaved errors.",
        ),
        (
            "PLSR calibration fit",
            "Fit y_pred = a + b y_true. Why: calibration-in-the-large. Read: a≈0 and b≈1 indicate unbiased predictions; deviations signal bias.",
        ),
        (
            "Multivariate selection table",
            "CV comparison of models. Why: justify choice (PLSR/ICA/MCR-ALS). Read: selected model balances accuracy and simplicity under CV.",
        ),
        (
            "ROI performance (with bootstrap CIs)",
            "Sensitivity (slope), fit (R²), precision (RMSE), and detection limits with uncertainty. Why: quantify reliability. Read: narrower CIs → more stable performance.",
        ),
        (
            "MCR-ALS components / ROI overlay",
            "Resolved components and their alignment with ROI. Why: interpretability and spectral specificity. Read: component peaks aligning with ROI supports chemical plausibility.",
        ),
        (
            "ROI repeatability",
            "Variation across replicates. Why: assess stability. Read: small spread across repeats indicates robust sensor behavior.",
        ),
        (
            "Gas dashboard",
            "At-a-glance summary of key metrics. Why: operational decisions. Read: track improvements and regressions run-to-run.",
        ),
    ]
...
def _plain_language_md(run_dir: Path) -> str:
    ...
    try:
        ...
        if slope is not None:
            ...
            bullets.append(
                f"- The sensor signal {direction} as concentration increases; sensitivity (slope) ≈ {s:.3g} ΔT/ppm."
            )
        if r2 is not None:
            bullets.append(
                f"- Fit quality R² ≈ {float(r2):.3g}, indicating how much of the variation is explained by concentration."
            )
        if lod is not None and loq is not None:
            bullets.append(
                f"- Detection limits: LOD ≈ {float(lod):.3g} ppm and LOQ ≈ {float(loq):.3g} ppm (smaller is better)."
            )
        md.append(
            "This report shows where the spectrum responds to the gas, how strong and reliable the change is, and how accurately we can predict concentration.\n"
        )
        if bullets:
            md.append("\n".join(bullets) + "\n")
    except Exception:
...
def _glossary_md() -> str:
    items = [
        (
            ...
            "Wavelength window where the analyte causes the most consistent, strongest change.",
        ),
        (
            "FDR / q-value",
            "FDR controls false discoveries across many tests; q-value is the adjusted p-value after FDR.",
        ),
        (
            "Permutation test",
            "Shuffle concentrations to build a 'no-effect' baseline; compare the observed slope against this null.",
        ),
        (
            "PLSR",
            "Multivariate model that predicts concentration from the whole spectrum using a small number of components.",
        ),
        (
            "Predictive interval",
            "Range around the prediction expected to contain the true value with a given probability (e.g., 90%).",
        ),
        (
            "LOD / LOQ",
            "Smallest levels we can reliably detect/quantify, based on noise and sensitivity.",
        ),
        (
            "Calibration-in-the-large",
            "Checks overall prediction bias using y_pred = a + b·y_true; unbiased if a≈0 and b≈1.",
        ),
    ]
    md = []
...
def _key_findings_md(run_dir: Path) -> str:
    ...
    try:
        ...
        if s is not None:
            ...
            direction = "decreases" if s < 0 else "increases"
            bullets.append(
                f"- ROI signal {direction} with concentration; sensitivity (slope) ≈ {s:.3g} ΔT/ppm."
            )
        if r2 is not None and rmse is not None:
            bullets.append(f"- ROI fit quality: R² ≈ {float(r2):.3g}, RMSE ≈ {float(rmse):.3g} ΔT.")
        if lod is not None and loq is not None:
            bullets.append(
                f"- Detection limits: LOD ≈ {float(lod):.3g} ppm; LOQ ≈ {float(loq):.3g} ppm."
            )
    except Exception:
        pass
        ...
        if not tw.empty and "wavelength" in tw.columns:
            ...
            for _, r in tw[cols].head(3).iterrows():
                ...
                sig = (
                    f"q≈{float(q):.2g}"
                    if q is not None
                    else (f"p≈{float(p):.2g}" if p is not None else "")
                )
    ...
    try:
        ...
        if set(["y_true", "y_pred"]).issubset(df.columns) and not df.empty:
            ...
            cov95 = float(np.mean(np.abs(resid) <= q95))
            bullets.append(
                f"- PLSR calibration: slope b ≈ {b:.3g}, intercept a ≈ {a:.3g}; coverage: 90%→{cov90:.3f}, 95%→{cov95:.3f}."
            )
    except Exception:
        pass
    if bullets:
...
def _dual_channel_calibration_md(run_dir: Path) -> str:
    ...
    try:
        ...
        try:
            win_i = (
                f"{float(roi_s):.2f}–{float(roi_e):.2f} nm"
                if roi_s is not None and roi_e is not None
                else "NA"
            )
        except Exception:
            win_i = "NA"
        ...
        try:
            ...
            gsd = None
        s_lod = (
            (3.0 * gsd / abs(float(s_slope)))
            if (gsd is not None and s_slope not in (None, 0, 0.0))
            else None
        )
        s_loq = (
            (10.0 * gsd / abs(float(s_slope)))
            if (gsd is not None and s_slope not in (None, 0, 0.0))
            else None
        )
                ...
                if not df.empty:
                    ...
                    if "score" in cand.columns:
                        ...
                        if "r2_w" in r.columns and "slope_w" in r.columns:
                            r["tmp"] = r["r2_w"].fillna(0).astype(float) * r[
                                "slope_w"
                            ].abs().fillna(0).astype(float)
        ...
        try:
            if i_slope is not None and i_int is not None:
                eq_lines.append(
                    f"- Intensity: T = {float(i_int):.5g} + {float(i_slope):.5g}·c (ΔT/ppm; ROI {win_i})."
                )
        except Exception:
            pass
        try:
            if s_slope is not None and s_int is not None:
                eq_lines.append(
                    f"- Wavelength shift: Δλ = {float(s_int):.5g} + {float(s_slope):.5g}·c (nm/ppm; window {win_shift})."
                )
        except Exception:
            pass
        if eq_lines:
...
def _units_md() -> str:
    ...
    items = [
        ...
        (
            "Predictive interval",
            "Half-width in ppm, derived from residual quantiles (empirical).",
        ),
    ]
    for k, v in items:
        md.append(f"- **{k}**: {v}")
    ...
    try:
        ...
        try:
            pypandoc.convert_file(
                str(md_path), "pdf", outputfile=str(pdf_path)
...
def _recommendations_md(run_dir: Path) -> str:
    ...
    if snr_msg:
        bullets.append(
            f"Improve SNR before claiming tight LOD/LOQ. {snr_msg} Consider optics shielding, thermal control, longer averaging, and illumination alignment."
        )
    else:
        bullets.append(
            "Improve SNR via optics shielding, temperature control, longer averaging, and illumination alignment."
        )

    if roi_hint:
        bullets.append(
            f"Anchor ROI using top STN wavelengths (approx. {roi_hint[0]:.1f}–{roi_hint[1]:.1f} nm). Validate with derivative+SNV and baseline settings."
        )
    else:
        bullets.append(
            "Use top_wavelengths.csv to anchor ROI center and bandwidth; validate preprocessing (derivative+SNV, baseline)."
        )

    if best_method:
        bullets.append(
            f"Multivariate: prioritize {best_method.upper()} based on CV R²; keep PLSR as exploratory with one-SE rule and CV ribbons."
        )
    else:
        bullets.append(
            "Multivariate: compare MCR-ALS, PLSR, ICA; use repeated CV with one-SE rule and report ±1 SE ribbons."
        )
    ...
    try:
        ...
        if t90 is not None and t10 is not None:
            bullets.append(
                f"Kinetics: T90 ≈ {float(t90):.0f}s, T10 ≈ {float(t10):.0f}s. Set dwell/purge times accordingly; consider preconditioning."
            )
    except Exception:
        pass
...
def build_report(
    run_dir: Path, title: Optional[str] = None, output_name: str = "report_full"
) -> Path:
...
def main():
    parser = argparse.ArgumentParser(
        description="Build PDF/HTML report for a gas analysis run directory."
    )
    parser.add_argument(
        "run_dir",
        type=str,
        help="Path to the run output directory (e.g., output/etoh_topavg)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="report_full",
        help="Base name for the output report",
    )
