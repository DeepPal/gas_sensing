...
def generate_results_summary(output_dir: str, target_metrics: Optional[dict] = None) -> str:
    ...
    if t90_mean is not None:
        ...
        md_lines.append(
            f"| **T90 (Response)** | **{t90_mean:.1f} ± {t90_std:.1f} s** | {target_metrics['t90']} s | {t90_mean / target_metrics['t90']:.1f}× | {t90_status} |"
        )
    else:
        md_lines.append(
            f"| **T90 (Response)** | **Not measured** | {target_metrics['t90']} s | - | - |"
        )

    if t10_mean is not None:
        t10_status = get_status(t10_mean, target_metrics["t10"], higher_is_better=False)
        md_lines.append(
            f"| **T10 (Recovery)** | **{t10_mean:.1f} ± {t10_std:.1f} s** | {target_metrics['t10']} s | {t10_mean / target_metrics['t10']:.1f}× | {t10_status} |"
        )
    else:
        md_lines.append(
            f"| **T10 (Recovery)** | **Not measured** | {target_metrics['t10']} s | - | - |"
        )
    ...
    if r2 >= target_metrics["r2"]:
        md_lines.append(
            f"✅ **Exceptional linearity**: R² = {r2:.4f} exceeds benchmark ({target_metrics['r2']:.2f})"
        )
    else:
        md_lines.append(
            f"⚠️ **Linearity**: R² = {r2:.4f} below benchmark ({target_metrics['r2']:.2f})"
        )

    if slope >= target_metrics["slope"] * 0.8:
        md_lines.append(
            f"✅ **Good sensitivity**: Slope = {slope:.4f} nm/ppm ({slope_pct:.0f}% of target)"
        )
    else:
        md_lines.append(
            f"⚠️ **Limited sensitivity**: Slope = {slope:.4f} nm/ppm ({slope_pct:.0f}% of target, likely hardware-limited)"
        )
    ...
    if t90_mean and t90_mean <= target_metrics["t90"] * 1.5:
        ...
        md_lines.append(
            f"⚠️ **Slower response**: T90 = {t90_mean:.1f} s (target: {target_metrics['t90']} s)"
        )
    ...
    if pub_score >= 3:
        md_lines.append(
            "**Status**: ✅ **Publication-ready** for tier-2 journals (Sensors, Chemosensors)"
        )
        md_lines.append("")
